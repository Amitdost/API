...
class UserApi extends CI_Controller {
   ...
   /* public function updateusermoney(){
    ...
    public function upadateUserSetting(){

        $token      = $this->token;

        $user_id                = $this->input->post('user_id');
        $language               = $this->input->post('language');
        $primary_language       = $this->input->post('primary_language');
        $secondary_language     = $this->input->post('secondary_language');
        $lowmoney_value         = $this->input->post('lowmoney_value');
        $lowmoney_status        = $this->input->post('lowmoney_status');
        $lowmoney_tune          = $this->input->post('lowmoney_tune');
        $lowmoney_alert         = $this->input->post('lowmoney_alert');
        $selfie_type            = $this->input->post('selfie_type');
        $audio_moneymanagement  = $this->input->post('audio_moneymanagement');
        $audio_shopping         = $this->input->post('audio_shopping');
        $audio_moneyihave       = $this->input->post('audio_moneyihave');
        $audio_goshopping       = $this->input->post('audio_goshopping');
        $shoppinglist_type      = $this->input->post('shoppinglist_type');
        ...
        if( $user_id != $token['user_id'] ){
            ...
            if( $language ){
                ...
                if( $language != 'single' && $language != 'dual' ){

                    $error = array( 'status' =>0, 'message' => 'Language type invalid');
                    echo json_encode($error);
                    exit;
                }else{
                        $addeddata['language']              = $language;
                        
                        if( $language == 'single' ){
                            $addeddata['primary_language'] = $primary_language;
                        }else if( $language == 'dual' ){
                            $addeddata['secondary_language']    = $secondary_language;
                        }
                }
            }
            ...
            if( $lowmoney_status ){
                ...
                if( $lowmoney_status != 'tune' && $lowmoney_status != 'alarm' ){

                    $error = array( 'status' =>0, 'message' => 'Invalid low money status');
                    echo json_encode($error);
                    exit;
                }else{
                        $addeddata['lowmoney_status']           = $lowmoney_status;
                       
                        if( $language == 'tune' ){

                            $addeddata['lowmoney_tune']     = $lowmoney_tune;

                        }else if( $language == 'alarm' ){

                            $addeddata['lowmoney_alert']    = $lowmoney_alert;
            ...
            if( $selfie_type ){
                ...
                if( $selfie_type != 'same' && $selfie_type != 'independent' ){

                    $error = array( 'status' =>0, 'message' => 'Invalid selfie type');
                    echo json_encode($error);
                    exit;
                }else{
                        $addeddata['selfie_type']           = $selfie_type;
                }   
            }


            if( $shoppinglist_type ){

                if( $shoppinglist_type != 'repeated' && $shoppinglist_type != 'column' ){

                    $error = array( 'status' =>0, 'message' => 'Invalid shopping list type');
                    echo json_encode($error);
                    exit;
                }else{
                        $addeddata['shoppinglist_type']           = $shoppinglist_type;
                }   
            }



            if( $audio_moneymanagement ){

                if( $audio_moneymanagement != '0' && $audio_moneymanagement != '1' ){

                    $error = array( 'status' =>0, 'message' => 'Invalid Money Management Audio Type');
                    echo json_encode($error);
                    exit;
                }else{
                        $addeddata['audio_moneymanagement']           = $audio_moneymanagement;
                }   
            }
            ...
            if( $audio_shopping ){
                ...
                if( $audio_shopping != '0' && $audio_shopping != '1' ){

                    $error = array( 'status' =>0, 'message' => 'Invalid Shopping List Audio Type');
                    echo json_encode($error);
                    exit;
                }else{
                        $addeddata['audio_shopping']           = $audio_shopping;
                }   
            }
            ...
            if( $audio_moneyihave ){
                ...
                if( $audio_moneyihave != '0' && $audio_moneyihave != '1' ){

                    $error = array( 'status' =>0, 'message' => 'Invalid Money i have Audio Type');
                    echo json_encode($error);
                    exit;
                }else{
                        $addeddata['audio_moneyihave']           = $audio_moneyihave;
                }   
            }
            ...
            if( $audio_goshopping ){
                ...
                if( $audio_goshopping != '0' && $audio_goshopping != '1' ){

                    $error = array( 'status' =>0, 'message' => 'Invalid Go Shopping Audio Type');
                    echo json_encode($error);
                    exit;
                }else{
                        $addeddata['audio_goshopping']           = $audio_goshopping;
                }   
            }
            ...
            if( $lowmoney_value ){
                ...
                $addeddata['lowmoney_value']           = $lowmoney_value;
              
            }


            $addeddata['updated_on'] = time();

            $table = 'sh_setting';
            $condition  =   array( 'user_id' => $user_id );
            

            $updated = $this->basic_model->update( $table, $addeddata, $condition );
            ...
            if( $updated ){
                ...
                $error = array( 'status' => 1, 'message' => 'Try again');
                echo json_encode($error);
                exit;
            }
        }
